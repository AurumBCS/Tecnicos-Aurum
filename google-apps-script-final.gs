...
function servirVacaciones() {
  ...
  try {
    ...
    if(lines.length < 3) return ContentService.createTextOutput('mat,consumido,disponible').setMimeType(ContentService.MimeType.TEXT);
    ...
    const rows = [['mat', 'consumido', 'disponible']];
    ...
    for(let i = 2; i < lines.length; i++){
      ...
      rows.push([mat, consumido, disponible]);
    }

    const resultado = rows.map(r => r.map(c => '"' + String(c||'').replace(/"/g, '""') + '"').join(',')).join('\n');
    cache.put('vacaciones_data', resultado, 3600);
    Logger.log('✓ Vacaciones: ' + (rows.length - 1) + ' técnicos');
    return ContentService.createTextOutput(resultado).setMimeType(ContentService.MimeType.TEXT);

  } catch(e) {
    Logger.log('✗ Error: ' + e.toString());
    return ContentService.createTextOutput('mat,consumido,disponible').setMimeType(ContentService.MimeType.TEXT);
  }
}
...
// ════════════════════════════════════════════════════════
//  ONEDIT Y LIMPIAR CACHE
// ════════════════════════════════════════════════════════
function onEdit(e) {
  CacheService.getScriptCache().removeAll(['csv_data','docs_data','auth_data','flota_data','medalia_data','vacaciones_data','cumplimiento_data','rm_data']);
}
...
function servirMedialiaCoordinadores() {
  ...
  try {
    ...
    if(lines.length < 2) return ContentService.createTextOutput('territorio,promedio,cantidad').setMimeType(ContentService.MimeType.TEXT);
    ...
    const rows = [['territorio', 'promedio', 'cantidad']];
    Object.keys(grupos).forEach(g => {
      if(grupos[g].count > 0) {
        const promedio = (grupos[g].sum / grupos[g].count).toFixed(2);
        rows.push([g, promedio, grupos[g].count]);
      }
    });

    const resultado = rows.map(r => r.map(c => '"' + String(c||'').replace(/"/g, '""') + '"').join(',')).join('\n');
    cache.put('medalia_coordinadores_data', resultado, 3600);
    Logger.log('✓ Medalia coordinadores: ' + (rows.length - 1) + ' grupos');
    return ContentService.createTextOutput(resultado).setMimeType(ContentService.MimeType.TEXT);

  } catch(e) {
    Logger.log('✗ Error medalia: ' + e.toString());
    return ContentService.createTextOutput('territorio,promedio,cantidad').setMimeType(ContentService.MimeType.TEXT);
  }
}

// ════════════════════════════════════════════════════════
//  SERVIR CUMPLIMIENTO (CSV)
// ════════════════════════════════════════════════════════
function servirCumplimiento() {
  const cache = CacheService.getScriptCache();
  const cached = cache.get('cumplimiento_data');
  if(cached) return ContentService.createTextOutput(cached).setMimeType(ContentService.MimeType.TEXT);
  ...
  try {
    ...
    data.slice(1).forEach(row => {
      ...
      const razon = String(row[9]||'').trim();
      if(mat) rows.push([mat, territorial, averia, cumplido, razon]);
    });

    const resultado = rows.map(r => r.map(c => '"' + String(c||'').replace(/"/g, '""') + '"').join(',')).join('\n');
    cache.put('cumplimiento_data', resultado, 3600);
    Logger.log('✓ Cumplimiento: ' + (rows.length - 1) + ' registros');
    return ContentService.createTextOutput(resultado).setMimeType(ContentService.MimeType.TEXT);

  } catch(e) {
    Logger.log('✗ Error cumplimiento: ' + e.toString());
    return ContentService.createTextOutput('mat,territorial,averia,cumplido,razon').setMimeType(ContentService.MimeType.TEXT);
  }
}
...
function servirRM() {
  const cache = CacheService.getScriptCache();
  const cached = cache.get('rm_data');
  if(cached) return ContentService.createTextOutput(cached).setMimeType(ContentService.MimeType.TEXT);
  ...
  try {
    ...
    data.slice(1).forEach(row => {
      ...
      const averia = String(row[5]||'').trim();
      if(nMant && tecnico) rows.push([nMant, motivo, tecnico, territorial, repetido, motivo_post, averia]);
    });

    const resultado = rows.map(r => r.map(c => '"' + String(c||'').replace(/"/g, '""') + '"').join(',')).join('\n');
    cache.put('rm_data', resultado, 3600);
    Logger.log('✓ RM: ' + (rows.length - 1) + ' registros');
    return ContentService.createTextOutput(resultado).setMimeType(ContentService.MimeType.TEXT);

  } catch(e) {
    Logger.log('✗ Error RM: ' + e.toString());
    return ContentService.createTextOutput('nMant,motivo,tecnico,territorial,repetido,motivo_post,averia').setMimeType(ContentService.MimeType.TEXT);
  }
}
